...
const getThongKeKhachHang = async (req, res) => {
    ...
    try {
      ...
      if (!startYear && !endYear) {
        const yearsRange = await PhieuXuat.findAll({
          attributes: [
            [sequelize.fn("MIN", sequelize.fn("YEAR", sequelize.col("thoi_gian_xuat"))), "minYear"],
            [sequelize.fn("MAX", sequelize.fn("YEAR", sequelize.col("thoi_gian_xuat"))), "maxYear"],
          ],
          raw: true,
        });
        ...
        if (yearsRange.length === 0) {
          return res.status(404).json({ message: "Không tìm thấy dữ liệu năm trong database." });
        }
    ...
    if (startYear && !endYear) {
     ...
     if (!startYear && endYear) {
      const minYearData = await PhieuXuat.findOne({
        attributes: [[sequelize.fn("MIN", sequelize.fn("YEAR", sequelize.col("thoi_gian_xuat"))), "minYear"]],
        raw: true,
      });
      ...
      const results = await Promise.all(
        years.map(async (year) => {
            ...
            include: [
              {
                ...
                as:'phieuNhap',
                attributes: [],
                where: sequelize.where(sequelize.fn("YEAR", sequelize.col("thoi_gian_nhap")), year),
              },
            ],
            raw: true,
          });
          ...
          const doanhthu = await chiTietPhieuXuatModel.findAll({
            attributes: [[sequelize.literal("SUM(so_luong*gia_xuat)"), "doanhthu"]],
            include: [
              {
                model: PhieuXuat,
                as:'phieuXuat',
                attributes: [],
                where: sequelize.where(sequelize.fn("YEAR", sequelize.col("thoi_gian_xuat")), year),
              },
            ],
            raw: true,
          });
  ...
  const getThongKeTheoThang = async (req,res) =>{
    ...
    if (!Year) {
      const latestYearData = await PhieuXuat.findOne({
        attributes: [[sequelize.fn("MAX", sequelize.fn("YEAR", sequelize.col("thoi_gian_xuat"))), "latestYear"]],
        raw: true,
      });

      // Nếu không tìm thấy dữ liệu
      if (!latestYearData || !latestYearData.latestYear) {
        return res.status(404).json({ message: "Không tìm thấy dữ liệu năm trong cơ sở dữ liệu." });
      }
            ...
            include: [
              {
                ...
                where: {
                  [Op.and]: [
                    sequelize.where(sequelize.fn("YEAR", sequelize.col("thoi_gian_nhap")), year),
                    sequelize.where(sequelize.fn("MONTH", sequelize.col("thoi_gian_nhap")), month),
                  ],
                },
              },
            ],
            ...
            include: [
              {
                ...
                where: {
                  [Op.and]: [
                    sequelize.where(sequelize.fn("YEAR", sequelize.col("thoi_gian_xuat")), year),
                    sequelize.where(sequelize.fn("MONTH", sequelize.col("thoi_gian_xuat")), month),
                  ],
                },
              },
            ],
  ...
  const getThongKeTheoNgay = async (req,res) =>{
    ...
    const Month = month ? parseInt(month, 10) : today.getMonth() + 1;
      if ( Month < 1 || Month > 12) {
        return res.status(400).json({ message: "Vui lòng cung cấp year và month hợp lệ (month: 1-12)." });
      }
     // Đảm bảo chỉ lấy các ngày từ ngày 1 đến ngày cuối cùng của tháng
      ...
      const days = Array.from(
        { length: daysInMonth },
          (_, i) => `${Year}-${String(Month).padStart(2, '0')}-${String(i + 1).padStart(2, '0')}` // Tạo danh sách ngày định dạng YYYY-MM-DD
      );
